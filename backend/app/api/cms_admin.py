...
from app.models import CmsAnnouncement, CmsHomePage
...
def _get_or_create_announcement() -> CmsAnnouncement:
    """单条公告：按 id 固定顺序，避免多行时 first() 不确定导致读写到不同行。"""
...
def _serialize_announcement(a: CmsAnnouncement) -> dict:
    ...
    return {
        ...
        "status": _status_to_client(a.status),
        "viewCount": a.view_count or 0,
        "lastViewAt": isoformat_utc(a.last_view_at),
        "lastViewUserId": a.last_view_mobile_user_id,
